export function setupViewers(roomId){
  ...
  fetch(`/static/indices/${level}.json`).then(r=>r.ok?r.json():null).then(idx=>{
    if(!idx){ const t=document.getElementById("fr-toast"); if(t) t.hidden=false; return; }
    const f=(idx.rooms||[]).find(x=>x.id===roomId);
    if(!f){ const t=document.getElementById("fr-toast"); if(t) t.hidden=false; return; }
    if(fr) fr.innerHTML = `<div class="placeholder">Auto-zoomed ${level}</div>`;
  }).catch(()=>{ const t=document.getElementById("fr-toast"); if(t) t.hidden=false; });
}
